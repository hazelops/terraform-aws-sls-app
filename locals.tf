locals {
  ...
  } : length(var.private_subnets) > 1 ? {for k, v in var.private_subnets : "private_subnet_${k + 1}" => v} : {for k, v in var.private_subnets : "private_subnet" => v}

  public_subnets = var.serialize_lists ? {
    public_subnets = var.join_lists ? join(",", var.public_subnets) : jsonencode(var.public_subnets)
  } : length(var.public_subnets) > 1 ? {for k, v in var.public_subnets : "public_subnet_${k + 1}" => v} : {for k, v in var.public_subnets : "public_subnet" => v}
  ...
  } : length(var.security_groups) > 1 ? {for k, v in var.security_groups : "security_group_${k + 1}" => v} : {for k, v in var.security_groups : "security_group" => v}
  ...
  parameters = merge(
    var.parameters, local.private_subnets, local.public_subnets, local.security_groups, local.vpc
  )
}
